...
Given that the value of the computed *p*-value (*p* = 0.21) exceeds our pre-chosen significance threshold of 0.05, we reject our null hypothesis that the perturbation does not affect gene expression. In practice, when analyzing a large dataset, we would apply a Benjamini-Hochberg correction to all gene-gRNA pairs and use a more conservative threshold to determine significance.
