...
#' @param theta the vector of unregularized gene dispersions
...
#' bw_adjust <- 3
#' theta_reg <- regularize_thetas(genes_log_gmean, theta, theta_regularization, 0)
...
regularize_thetas <- function(genes_log_gmean, theta, theta_regularization = "log_theta", bw_adjust = 3, plot_me = FALSE) {
  if (bw_adjust == 0) {
    ...
    fitted_dispersion_par[o] <- ksmooth(x = genes_log_gmean_step1, y = dispersion_par, x.points = x_points, bandwidth = bw, kernel = "normal")$y
    ...
    if (plot_me) {
      df1 <- tibble(log_theta = c(dispersion_par, fitted_dispersion_par), mean_gene_exp = c(genes_log_gmean_step1, x_points), regularized = c(rep(FALSE, length(genes_log_gmean_step1)), rep(TRUE, length(x_points))))
      p1 <- ggplot(data = df1, mapping = aes(x = mean_gene_exp, y = log_theta, col = regularized)) + geom_point() + theme_bw()
      df2 <- tibble(theta = theta, theta_out = theta_out)
      p2 <- ggplot(data = df2, mapping = aes(x = theta, y = theta_out)) + geom_point() + theme_bw()
      print(p1); print(p2)
    }
  }
  return(theta_out)
...
#' Internal sctransform function
robust_scale <- function(x) {(x - median(x))/(mad(x) + .Machine$double.eps)}
