...
p_vals_sceptre <- paste0(results_dir, "/all_results_with_names.fst") %>% read.fst() %>% as_tibble() %>% filter(enh_names == "ARL15-enh") %>% select(p_value, gene_names) %>% mutate(p_value_adj = p.adjust(p_value, method = "BH"))
...
p_vals_sceptre %>% arrange(p_value_adj) %>% head()
p_vals_bulk %>% arrange(p_value_adj) %>% head() 
