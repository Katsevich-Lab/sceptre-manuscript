...
f_name <- paste0("sim_res_", if (!is.na(theta_size)) paste0(theta_size,"_") else "", method)
...
p_vals <- map_dbl(.x = 1:n_rep, .f = function(i) {
  ...
  # Run the method
  if (method == "sceptre") {
    out <- run_sceptre_gRNA_gene_pair(expressions = expressions, gRNA_indicators = gRNA_indicators, covariate_matrix = covariate_matrix, gene_precomp_size = theta, B = B, seed = seed) %>% pull(p_value)
    ...
    lib_sizes <- as.integer(exp(covariate_matrix$lg_total_umis))
